...
function MouseMove(e) {
  var i, tc, o, tg = e.target && Defined(e.target.id) ?
    e.target.id : e.srcElement.parentNode.id;
...
TCproto.Animate = function(w,h) {
  ...
  if(x >= 0 && y >= 0 && x < w && y < h)
  {
    ...
    tc.lx || (this.yaw = r * ((s * 2 * x / w) - s));
    tc.ly || (this.pitch = r * -((s * 2 * y / h) - s));
    this.initial = null;
  }
  else if(!tc.initial)
  {
...
jQuery.fn.tagcanvas = function(options,lctr) {
  ...
  this.each(function() {
    ...
    jqt.lx = (jqt.lock + '').indexOf('x') + 1;
    jqt.ly = (jqt.lock + '').indexOf('y') + 1;
